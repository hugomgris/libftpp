...
/**
 * @brief ObservableValue notifies subscribers when the value is modified
 * 
...
public:
    ...
    bool trySet(const T& newValue); // Returns false if validation fails
...
/**
 * @brief Specialized observable for numeric values with additional operations
 */
template<typename T>
...
public:
    ...
    using ObservableValue<T>::operator=; // Inherit assignment operators
    ...
    ObservableNumber& operator++(); // Prefix
    ObservableNumber operator++(int); // Postfix
    ObservableNumber& operator--(); // Prefix
    ObservableNumber operator--(int); // Postfix
...
/**
 * @brief Observable container for collections
 */
